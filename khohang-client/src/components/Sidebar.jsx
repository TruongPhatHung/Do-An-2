...
const Sidebar = () => {
    ...
    const [openMenus, setOpenMenus] = useState({
        danhMuc: true,
        kinhDoanh: isKinhdoanh || isAdmin,
        muaHang: isMuaHang || isAdmin,
        kho: isKho || isAdmin,
        quanTri: isQuanLyKho || isAdmin
    });
    ...
    return (
        <aside className="sidebar">
            ...
            <ul className="sidebar-menu">
                ...
                {/* 🎯 [UML: Xem tổng quan báo cáo] - Dành cho QUẢN LÝ KHO & ADMIN */}
                {(isAdmin || isQuanLyKho) && (
                    <>
                ...
                {/* 🎯 [UML: NV Kinh Doanh -> Lập YC xuất/nhập, Xem lịch sử] */}
                {(isAdmin || isKinhdoanh) && (
                    <li>
                ...
                {/* 🎯 [UML: NV Mua Hàng -> Lập PO, Quản lý DS PO] */}
                {(isAdmin || isMuaHang) && (
                    <li>
                        ...
                        {openMenus.muaHang && (
                            <ul className="sidebar-submenu">
                                <li><NavLink to="/don-hang" className={navClass}><FiClipboard /> Lập Đơn Hàng (PO)</NavLink></li>
                ...
                {/* --- 4. NGHIỆP VỤ KHO VẬN --- */}
                {/* 🎯 [UML: NV Kho -> Xuất/Nhập kho, Xử lý đơn giao thiếu, LS Giao dịch] */}
                {(isAdmin || isKho) && (
                    <li>
                ...
                {/* --- 5. QUẢN TRỊ & PHÊ DUYỆT --- */}
                {/* 🎯 [UML: Quản lý Kho -> Duyệt YC Xuất/Nhập] & [Admin -> Tài khoản, Logs] */}
                {(isAdmin || isQuanLyKho) && (
                    <li>
                        <div className={`sidebar-dropdown-header ${openMenus.quanTri ? 'open' : ''}`} onClick={() => toggleMenu('quanTri')}>
                            <div className="header-left">
                                <FiShield className="menu-icon-svg" /> <span>Quản Trị</span>
                            </div>
                            <FiChevronDown className="arrow-icon" />
                        </div>
                        {openMenus.quanTri && (
                            <ul className="sidebar-submenu">
                                ...
                                {isAdmin && (
                                    <>
                                        <li className="submenu-label">Hệ thống (Chỉ Admin)</li>
                                        <li><NavLink to="/tai-khoan" className={navClass}><FiUsers /> Quản Lý Tài Khoản</NavLink></li>
                                        <li><NavLink to="/admin/logs" className={navClass}><FiActivity /> Logs Thao Tác</NavLink></li>
                                    </>
                                )}
                            </ul>
                        )}
